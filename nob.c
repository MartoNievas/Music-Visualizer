...
bool delete_walk_entry(Walk_Entry entry) { return delete_file(entry.path); }
...
bool build_and_run_test(const char *test_name, bool record) {
...
#ifdef _WIN32
  const char *src_stdout_path = temp_sprintf(
      "%s%s.win32.stdout.txt", BUILD_FOLDER TESTS_FOLDER, test_name);
  const char *dst_stdout_path =
      temp_sprintf("%s%s.win32.stdout.txt", TESTS_FOLDER, test_name);
  const char *test_stdout_path =
      temp_sprintf("../%s.win32.stdout.txt", test_name);
#else
  const char *src_stdout_path =
      temp_sprintf("%s%s.stdout.txt", BUILD_FOLDER TESTS_FOLDER, test_name);
  const char *dst_stdout_path =
      temp_sprintf("%s%s.stdout.txt", TESTS_FOLDER, test_name);
  const char *test_stdout_path = temp_sprintf("../%s.stdout.txt", test_name);
#endif // _WIN32
  const char *bin_path =
      temp_sprintf("%s%s", BUILD_FOLDER TESTS_FOLDER, test_name);
  const char *src_path = temp_sprintf("%s%s.c", TESTS_FOLDER, test_name);
  const char *test_cwd_path =
      temp_sprintf("%s%s%s.cwd", BUILD_FOLDER, TESTS_FOLDER, test_name);
  ...
  if (file_exists(test_cwd_path)) {
    if (!delete_directory_recursively(test_cwd_path))
      return_defer(false);
  }
...
#endif // _WIN32
  if (!cmd_run(&cmd, .stdout_path = test_stdout_path))
    return_defer(false);
  if (!set_current_dir("../../../"))
    return_defer(false);
  ...
  if (record) {
    if (!copy_file(src_stdout_path, dst_stdout_path))
      return_defer(false);
  } else {
    // TODO: it would be cool to have a portable diff utility in here.
    if (!read_entire_file(src_stdout_path, &src))
      return_defer(false);
    if (!read_entire_file(dst_stdout_path, &dst))
      return_defer(false);
...
int main(int argc, char **argv) {
  ...
  if (command(command_name, &commands, "test", "[test_names...]",
              "Run the tests checking their expected output")) {
    if (!mkdir_if_not_exists(BUILD_FOLDER))
      return 1;
    if (!mkdir_if_not_exists(BUILD_FOLDER TESTS_FOLDER))
      return 1;
    ...
    if (argc <= 0) {
      for (size_t i = 0; i < test_names_count; ++i) {
        ...
        if (!build_and_run_test(test_names[i], false))
          failed_count += 1;
        temp_rewind(mark);
      }
    } else {
      while (argc > 0) {
        ...
        if (!build_and_run_test(test_name, false))
          failed_count += 1;
        temp_rewind(mark);
      }
    }
  ...
  if (command(command_name, &commands, "record", "[test_names...]",
              "Record expected output of the tests")) {
    if (!mkdir_if_not_exists(BUILD_FOLDER))
      return 1;
    if (!mkdir_if_not_exists(BUILD_FOLDER TESTS_FOLDER))
      return 1;
    ...
    if (argc <= 0) {
      for (size_t i = 0; i < test_names_count; ++i) {
        size_t mark = temp_save();
        if (!build_and_run_test(test_names[i], true))
          failed_count += 1;
        temp_rewind(mark);
      }
    } else {
      while (argc > 0) {
        size_t mark = temp_save();
        const char *test_name = shift(argv, argc);
        if (!build_and_run_test(test_name, true))
          failed_count += 1;
        temp_rewind(mark);
      }
    }
  ...
  if (command(command_name, &commands, "list", "", "List available tests")) {
    ...
    nob_log(INFO, "Use %s test <names...> to run individual tests",
            program_name);
    return 0;
  }
